...
mkdir -p disc
cp "/path/to/cdrom.dat" disc/cdrom.dat
```
...
- `make decompile FUNC=<symbol>` - Decompile a function to C using m2c
- `make asm-diff FUNC=<symbol>` - Show a TARGET vs BASE instruction diff table
- `make permute FUNC=<symbol>` - Run the permuter on a function
